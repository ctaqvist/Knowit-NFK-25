...
async def test_process_command_start_stream():
    websocket = MagicMock()
    with patch("RaspberryPI.commands.rover.handler.handle_stream_command") as mock_stream:
...
async def test_process_command_stop_stream():
    websocket = MagicMock()
    with patch("RaspberryPI.commands.rover.handler.handle_stop_stream_command") as mock_stop:
...
async def test_process_command_light():
    websocket = MagicMock()
    with patch("RaspberryPI.commands.rover.handler.handle_light_command") as mock_light:
